...
class TipoAjudaController extends Controller
{
    ...
    public function destroy(string $id): void { $tipos = new TipoAjudaRepository(); if ($tipos->contarEntregas((int) $id) > 0) { Session::flash('warning','Não é possível excluir um tipo de ajuda que possui entregas registradas. Inative o item para preservar o histórico.'); $this->redirect('/tipos-ajuda'); return; } $tipos->excluir((int)$id); Session::flash('success','Tipo de ajuda excluído.'); $this->redirect('/tipos-ajuda'); }
}
