...
CREATE TEMPORARY TABLE IF NOT EXISTS tmp_desastre_cobrade_codigo (desastre_id BIGINT UNSIGNED PRIMARY KEY, codigo VARCHAR(50) NOT NULL);
DELETE FROM tmp_desastre_cobrade_codigo;
INSERT INTO tmp_desastre_cobrade_codigo (desastre_id, codigo) SELECT d.id, cs.codigo FROM desastres d INNER JOIN cobrade_subtipos cs ON cs.id = d.cobrade_subtipo_id;

SET FOREIGN_KEY_CHECKS = 0;
DELETE FROM cobrade_subtipos;
DELETE FROM cobrade_tipos;
DELETE FROM cobrade_subgrupos;
DELETE FROM cobrade_grupos;
...
UPDATE desastres d INNER JOIN tmp_desastre_cobrade_codigo tmp ON tmp.desastre_id = d.id INNER JOIN cobrade_subtipos cs ON cs.codigo = tmp.codigo SET d.cobrade_subtipo_id = cs.id;
DROP TEMPORARY TABLE IF EXISTS tmp_desastre_cobrade_codigo;
